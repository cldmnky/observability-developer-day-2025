...
echo ""
echo "Adding two annotations to each deployment:"
echo "  1. sidecar.opentelemetry.io/inject: sidecar"
echo "  2. instrumentation.opentelemetry.io/inject-<lang>: demo-instrumentation"

pause

explain "Patching go-api deployment..."
run_cmd "oc patch deployment go-api -n observability-demo -p '{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"sidecar.opentelemetry.io/inject\":\"sidecar\"}}}}}'"

explain "Patching python-api deployment..."
run_cmd "oc patch deployment python-api -n observability-demo -p '{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"sidecar.opentelemetry.io/inject\":\"sidecar\",\"instrumentation.opentelemetry.io/inject-python\":\"demo-instrumentation\"}}}}}'"

explain "Patching quarkus-api deployment..."
run_cmd "oc patch deployment quarkus-api -n observability-demo -p '{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"sidecar.opentelemetry.io/inject\":\"sidecar\",\"instrumentation.opentelemetry.io/inject-java\":\"demo-instrumentation\"}}}}}'"

explain "Patching node-app deployment..."
run_cmd "oc patch deployment node-app -n observability-demo -p '{\"spec\":{\"template\":{\"metadata\":{\"annotations\":{\"sidecar.opentelemetry.io/inject\":\"sidecar\",\"instrumentation.opentelemetry.io/inject-nodejs\":\"demo-instrumentation\"}}}}}'"
...
echo "  ✓ Enabled distributed tracing (2 annotations per app)"
...
echo -e "${CYAN}Annotations added: 2 per deployment${NC}"
echo ""
echo "========================================="
echo ""
echo "Resources:"
